...
- 当前版本：`V0.1 工程基线版`
- 当前迭代：`I0 新项目工程基线`
- 当前成果：Java 25 后端多模块工程、多环境配置、MySQL／RabbitMQ／Redis 本地编排、Flyway 空库迁移、共享 Testcontainers 测试基座和 Spring Security 最小安全基线
- 当前策略：新项目从零建设；旧项目只作业务和技术参考，不复制旧代码和旧数据

当前仓库尚未进入业务功能实现阶段。I0 已完成后端工程骨架、多环境配置、三类本地基础设施编排、Flyway 空库迁移、共享 Testcontainers 测试基座和 Spring Security 最小安全基线；下一步建立前端 pnpm 工作区和 admin 应用。
