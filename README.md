...
- 当前版本：`V0.1 迭代开发版`
- 当前迭代：`I1 仓储主数据（I1.1 仓库主数据后端闭环）`
- 当前成果：I0 工程基线与仓库级统一命令已经验收；I1.1 已完成仓库分类、创建、详情、分页查询、启停、受控修改、审计、乐观并发控制、数据库迁移和接口错误闭环
- 当前策略：新项目从零建设；旧项目只作业务和技术参考，不复制旧代码和旧数据

当前仓库已经从 I0 工程基线进入 I1 主数据实施阶段。I1.1 只交付仓库主数据后端能力，不包含前端页面、库区、库位、库存、入库、出库或完整身份权限体系；后续切片仍须先确认业务范围和验收标准。
...
1. 完成 I1.1 仓库主数据后端切片的交付审阅。
2. 基于 I1.1 验收结果确认下一最小切片，不默认提前建设库区、库位或库存业务。
3. 按重新确认的业务规则实现第一个可验收 WMS 业务闭环。
